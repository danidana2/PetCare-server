...
public class ObesityManagementController {
    ...
    @PatchMapping("/{petId}/daily-calorie")
    public ResponseEntity<ResponseDTO<Object>> updateDailyCalorieResult(@PathVariable String petId, @RequestBody UpdateDailyCalorieResultDTO updateDailyCalorieResultDTO) {

        try {
            boolean updateResult = obesityManagementService.updateDailyCalorieResult(petId, updateDailyCalorieResultDTO);

            ResponseDTO<Object> response = new ResponseDTO<>(HttpStatus.OK.value(), "Obesity-degree, daily-calorie update successful", null);
            return ResponseEntity.ok(response);
        } catch (InvalidInputException e) {
            ...
            ResponseDTO<Object> response = new ResponseDTO<>(HttpStatus.INTERNAL_SERVER_ERROR.value(), "Obesity-degree, daily-calorie update failed", null);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
        }
    }
